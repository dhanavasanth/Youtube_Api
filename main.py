...
if choice == "SQL_QUERY's":
    ...
    if select == "What are the names of all the channels that have published videos in the year 2022?":
        cursor.execute("SELECT DISTINCT channel_name FROM channel_table WHERE strftime('%Y', Published_At) = '2022';")
        df = pd.DataFrame(cursor.fetchall(),columns=['channel_name','Publish_At'])
        if len(df) == 0:
            st.subheader("There was no channel created at the selected year")
            cursor.execute("SELECT DISTINCT channel_name,Published_At FROM channel_table ORDER BY Published_At DESC;")
            df = pd.DataFrame(cursor.fetchall(),columns=['channel_name','Publish_At'])
            st.dataframe(df)
        else:
            st.dataframe(df)    
